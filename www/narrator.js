/* ═══════════════════════════════════════════════════════════
   NARRATOR.JS — Audiobook narrator for Al-Saaiqa
...
(function() {
  ...
  const STATE = {
    ...
    cardIndex: 0,
    cards: [],
    tabOrder: ['about','truths','timeline'],
    tabIndex: 0,
  ...
  function extractCards(panel) {
    ...
    // Section title + subtitle (not for about tab — about has its own structure)
    if (tabName !== 'about') {
      const title = panel.querySelector('.section-title');
      const desc = panel.querySelector('.section-subtitle');
      if (title) {
        cards.push({
          el: title.closest('.tab-panel') || title,
    ...
    } else if (tabName === 'truths') {
      panel.querySelectorAll('.essay-card').forEach(el => {
        if (el.style.display === 'none') return;
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'essay' });
      });
    } else if (tabName === 'timeline') {
      panel.querySelectorAll('.timeline-event, .timeline-item, .timeline-entry').forEach(el => {
        const text = cleanText(el.textContent);
        if (text) cards.push({ el, text, type: 'timeline' });
      });
  ...
  function cleanText(text) {
    ...
    if (l === 'fr') {
      clean = clean
        ...
        .replace(/Al-Saaiqa/gi, 'al Sa-i-ka')
    ...
    if (l === 'en') {
      clean = clean
        ...
        .replace(/Al-Saaiqa/gi, 'al Sah-ee-kah')
  ...
  function readCurrentCard() {
    ...
    // Open essay cards so content is visible
    if (card.type === 'essay' && card.el && !card.el.classList.contains('open')) {
  ...
  function readDuoTranslation(card, onEnd) {
    ...
    // Try to get structured data for essays
    if (card.type === 'essay' && typeof TRUTHS !== 'undefined') {
      const essayCards = Array.from(card.el.parentNode.querySelectorAll('.essay-card'));
      const idx = essayCards.indexOf(card.el);
      if (idx >= 0 && TRUTHS[idx] && TRUTHS[idx][duoLang]) {
        const t = TRUTHS[idx][duoLang];
        duoText = (t.title || '') + '. ' + (t.text || t.desc || '');
      }
    }
  ...
  function setupMediaSession() {
    ...
    navigator.mediaSession.metadata = new MediaMetadata({
      title: l === 'ar' ? 'الصاعقة' : l === 'fr' ? 'La Foudre' : 'The Thunderbolt',
  ...
  function onSpeedChange(val) {
    ...
    localStorage.setItem('saaiqa-narrator-speed', STATE.speed);
  }
  ...
  function onPitchChange(val) {
    ...
    localStorage.setItem('saaiqa-narrator-pitch', STATE.pitch);
  }
  ...
  function onKaraokeToggle(checked) {
    ...
    localStorage.setItem('saaiqa-narrator-karaoke', checked);
  }

  function onAutoScrollToggle(checked) {
    STATE.autoScroll = checked;
    localStorage.setItem('saaiqa-narrator-autoscroll', checked);
  }

  function onDuoToggle(checked) {
    STATE.duoReading = checked;
    localStorage.setItem('saaiqa-narrator-duo', checked);
  }
  ...
  function loadSettings() {
    STATE.speed = parseFloat(localStorage.getItem('saaiqa-narrator-speed')) || 1;
    STATE.pitch = parseFloat(localStorage.getItem('saaiqa-narrator-pitch')) || 1;
    STATE.karaokeEnabled = localStorage.getItem('saaiqa-narrator-karaoke') !== 'false';
    STATE.autoScroll = localStorage.getItem('saaiqa-narrator-autoscroll') !== 'false';
    STATE.duoReading = localStorage.getItem('saaiqa-narrator-duo') === 'true';
  }
  ...
  function injectSpeakButtons() {
    ...
    var selectors = [
      '.about-card', '.essay-card', '.daily-card', '.stat-card', '.impact-quote-card'
    ];
  ...
  var origRenderDailyTruth = window.renderDailyTruth;
  if (origRenderDailyTruth) {
    window.renderDailyTruth = function() { origRenderDailyTruth(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderImpactQuotes = window.renderImpactQuotes;
  if (origRenderImpactQuotes) {
    window.renderImpactQuotes = function() { origRenderImpactQuotes(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderEssays = window.renderEssays;
  if (origRenderEssays) {
    window.renderEssays = function() { origRenderEssays(); setTimeout(injectSpeakButtons, 100); };
  }
  var origRenderTimeline = window.renderTimeline;
  if (origRenderTimeline) {
    window.renderTimeline = function() { origRenderTimeline(); setTimeout(injectSpeakButtons, 100); };
  }
